...
PR #2의 Phase 2 hardening과 Regulatory Updates MVP, PR #3의 Home V2, PR #5의 Home V3, PR #7의 Safety Intelligence MVP가 `main` production에 반영되었습니다. 현재 production release는 live Regulatory Updates, Safety Intelligence, CTD task를 우선하고, application/lifecycle guide는 source-backed workflow가 준비될 때까지 planned로 유지합니다.
...
- Public routes: `41` static/SSG routes

## Safety Intelligence production release

- Historical branch: `codex/safety-intelligence-mvp`
- Merged PR: `https://github.com/woojunxnam/regulatory-execution-hub/pull/7`
- Shared `OfficialSource` / `ChecklistItem` schema와 qualified-review validation 구현
- `Regulatory Updates`를 14개 record로 확장: 기존 guidance/procedure 8개 + Safety Intelligence 6개
- Safety coverage: FDA 2개, EMA 4개 official-source record
- Agency/category filter와 Safety Intelligence detail boundary 구현
- signal, causality, regulatory outcome, implementation status를 별도 controlled field로 유지
- affected document는 `required`, `conditional`, `recommended`, `potentially_impacted`, `best_practice`, `undetermined`로 명시적 분류
- Home task finder에서 Safety Intelligence route로 deterministic navigation 지원
- GitHub CI, Vercel production deployment, public desktop/mobile `90/90` 검증 완료
...
- 6개 Safety Intelligence official-source record와 category filter/detail route
...
Current Safety Intelligence production release:

- Formatting: pass
- ESLint zero warnings: pass
- strict TypeScript: pass
- Vitest: `13 files / 52 tests` pass
- Next.js production build: `41` routes pass
- Playwright: desktop/mobile `90/90` pass
...
| Safety Intelligence          | 6 official/source checked     | `source_checked`                                | 없음             |
...
- Safety Intelligence는 6개 curated record가 production에 구현됐으나 continuous monitoring/automatic publication은 없음
- FDA Initial IND, EMA Centralised MAA, post-approval change source-backed checklist 없음
- actual qualified `ReviewRecord` 없음

## 다음 작업

1. FDA Initial IND source package/checklist를 다음 bounded track으로 구현합니다.
2. Qualified RA reviewer를 확보하고 review record workflow를 실제 운영합니다.
3. 첫 5–10명 CMC/RA user interview와 usability test로 task priority를 검증합니다.
4. Monitoring cadence, superseded handling, correction ownership이 정해진 뒤 update detection automation을 설계합니다.
