...
## Safety Intelligence local release candidate

- Branch: `codex/safety-intelligence-mvp`
...
- 아직 production에는 반영되지 않았으며 PR/CI/Vercel 검증 후 merge 예정
...
- 6개 Safety Intelligence official-source record와 category filter/detail route (local release candidate)
...
- Safety Intelligence는 6개 curated record가 local release candidate에 구현됐으나 continuous monitoring/automatic publication은 없음
- FDA Initial IND, EMA Centralised MAA, post-approval change source-backed checklist 없음
- actual qualified `ReviewRecord` 없음

## 다음 작업

1. Safety Intelligence PR/CI/Vercel production deployment와 public URL 검증을 완료합니다.
2. FDA Initial IND source package/checklist를 다음 bounded track으로 구현합니다.
